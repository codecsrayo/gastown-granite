...
| hq-fe-auth.4 | Timeout + cleanup pty zombis + lock per account | P2 | closed | claude-host | `LoginConfig` gana `url_timeout_secs` / `token_timeout_secs` (env `GT_LOGIN_URL_TIMEOUT_SECS` / `GT_LOGIN_TOKEN_TIMEOUT_SECS`, defaults 30s / 300s); nuevo `LoginFailure::Timeout { phase }` en `gt-login::events`; watchdog en `gt-web::login::login_start` con `tokio::select` (sleep vs spawn_blocking handle); fase token hard deadline (driver bloquea en `rx.recv` → drop slot wakes); fase URL soft deadline (no kill-side handle en `Pty` port → driver despierta sólo cuando PTY EOF, slot queda libre igual); `event_sink` wrapper convierte `Failed{Cancelled}` → `Failed{Timeout{phase}}` cuando watchdog ya estampó la fase; `catch_unwind` en task de driver garantiza `registry.remove` + emite `Failed{Io{panicked}}` si el spawn_blocking panic; per-account lock = la mutex sync en `LoginRegistry::insert` (atomic check-then-insert ya prevenía race); 3 e2e tests nuevos (token-timeout→`Timeout{token}` typed, url-timeout soft path libera slot, panic guard emite Failed{Io}+slot clear); 164 tests gt-web verde |
| hq-fe-auth.5 | URL-phase hard-kill via PtyKiller port extension | P2 | closed | c3 | follow-up del TODO explícito de `.4`. Extiende `gt-login::Pty` con `PtyKiller` (`Send + Sync`, `kill()` idempotente) clonable via `PtyChild::killer()`; `PortableChild` agrega `Drop` que kill+wait reapa zombis si el driver task panic antes de `wait`. Nuevo `LoginDriver::run_with_timeouts(LoginTimeouts { url_phase, token_phase })` (defaults 120s/300s, `Duration::MAX` = unbounded) — watchdog thread por fase mantiene clone de `PtyKiller` y hard-kills al child al expirar; URL-phase EOF tras kill + `fired` flag → `Failed{Timeout{phase:"url"}}` (distingue de `UrlMissing`). `Watchdog::cancel_and_join` join-then-read `fired` resuelve la carrera caller-`recv_timeout` vs watchdog determinísticamente. `FakePty::blocking(...)` ctor (Condvar) drena script y queda bloqueado hasta el `PtyKiller` — drives el URL-hang test sin spawn real. `gt-web::login_start` ahora invoca `run_with_timeouts` directo; el `event_sink` wrapper `Cancelled→Timeout` de `.4` y la tokio-task watchdog se eliminan (driver emite typed `Failed{Timeout{phase}}` por sí mismo); token-source usa `rx.recv_timeout(token_phase)`. gt-terminal (otro `gt-login::Pty` consumer, hq-fe-term.1) actualizado en el mismo PR — 13 tests pasan sin cambio funcional. 16 tests gt-login + 10 tests gt-web `login_http` + 13 tests gt-terminal verdes. Main 8ede3d3c (PR #1). Cierra epic hq-fe-auth (5/5; .0 spike deferred). |
