...
/// Messages accepted by the Refinery actor. Scaffolding placeholder until behavior lands.
#[derive(Debug)]
#[non_exhaustive]
...
/// Cloneable handle to the Refinery actor. Producers and the composition root send
/// messages via [`RefineryHandle::send`].
#[derive(Clone)]
...
impl RefineryHandle {
    pub async fn send(&self, msg: RefineryMsg) {
        // Best-effort: dropping a message if the actor is gone matches the gt-merge pattern.
        let _ = self.tx.send(msg).await;
    }
}

/// Spawn the Refinery actor. **Scaffolding:** the loop drains messages without reacting
/// and never emits to the relay; the behavior commit replaces this body.
pub fn spawn(_events: mpsc::Sender<Envelope<RefineryEvent>>) -> RefineryHandle {
    let (tx, mut rx) = mpsc::channel::<RefineryMsg>(64);
    tokio::spawn(async move {
        while let Some(_msg) = rx.recv().await {
            // TODO(hq-92z9 fill): dispatch messages against RefineryBoard + emit events.
        }
    });
    RefineryHandle { tx }
}
