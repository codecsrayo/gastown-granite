...
impl EventKind for QuotaEvent {
    fn kind(&self) -> &'static str {
        match self {
            QuotaEvent::TokensSampled { .. } => "quota.tokens_sampled",
            QuotaEvent::UsageProbed { .. } => "quota.usage_probed",
            QuotaEvent::WindowReset { .. } => "quota.window_reset",
            QuotaEvent::BlockPredicted { .. } => "quota.block_predicted",
            QuotaEvent::AccountLimited { .. } => "quota.account_limited",
            QuotaEvent::Rotated { .. } => "quota.rotated",
            QuotaEvent::Blocked { .. } => "quota.blocked",
        }
    }
}
