...
impl EventKind for RigEvent {
    fn kind(&self) -> &'static str {
        match self {
            RigEvent::Added { .. } => "rig.added",
            RigEvent::Adopted { .. } => "rig.adopted",
            RigEvent::Removed { .. } => "rig.removed",
            RigEvent::PrefixChanged { .. } => "rig.prefix_changed",
            RigEvent::DefaultBranchChanged { .. } => "rig.default_branch_changed",
        }
    }
}
