...
impl<P: Pty + ?Sized> LoginDriver<P> {
    ...
    pub fn run_with_timeouts(
        ...
        //
        // `cancel_and_join` returns whether the watchdog fired so the race between
        // `token_source` returning `None` (caller hit its own deadline) and the watchdog
        // firing (we hit ours) resolves deterministically: joining the thread first
        // guarantees the `fired` flag is finalised before we read it.
        let token_watchdog = Watchdog::start(child.killer(), timeouts.token_phase);
        let token_result = token_source(&url_for_prompt);
        let token_timed_out = token_watchdog.cancel_and_join();
